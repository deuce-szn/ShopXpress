...
const ForYou = () => {
  ...
  const { loadCart,forYouProducts,forYouList } = useCartStore();

  const [isFilterButtonShow, setIsFilterButtonShow] = useState(false);
  ...
  const productsToUse =  forYouList;
  ...
  useEffect(() => {
    filterProducts();
  }, [checkedCategories, priceRange, checkedRatings, sortOption, forYouList]);
  ...
  const toggleFilterButton = () => setIsFilterButtonShow(prev => !prev);

  return (
    <div style={{ backgroundColor: "#f06040" }}
    onMouseEnter={toggleFilterButton}
    onMouseLeave={toggleFilterButton}
    
    >
     

      <div className='cateProducts-MainPage'>
